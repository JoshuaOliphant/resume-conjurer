# ABOUTME: In-memory mock ApplicationProvider — one fully tailored application from fixtures.
# ABOUTME: The stub adapter BACKEND.md replaces with a live Agent SDK backend; same contract.
"""Mock data provider for the Conjurer web prototype.

Shapes the real pipeline's output (an evidence pool, a chosen frame, units with
grounded variants) without a backend. The `_variant` builder resolves each cited
evidence id against EVIDENCE at construction, so a typo fails loudly here rather
than as a KeyError mid-render — and every `Variant` the UI sees carries valid,
resolved traces.
"""

from __future__ import annotations

from app.domain import Application, Evidence, Frame, Unit, Variant


# --- Evidence pool ---------------------------------------------------------
# Lines lifted from the (mock) master resume. Variants cite these by id, so every
# claim on screen traces back to something the candidate actually wrote.
...
def _variant(variant_id: str, text: str, *evidence_ids: str) -> Variant:
    """Build a Variant, resolving cited evidence ids against the pool.

    The trust mechanic depends on every cited id existing. Fail loudly here (when
    the fixtures are built) rather than with a KeyError mid-render in a template.
    """
    missing = [e for e in evidence_ids if e not in EVIDENCE]
    if missing:
        raise ValueError(f"Variant {variant_id!r} cites unknown evidence: {missing}")
    return Variant(id=variant_id, text=text, evidence=tuple(EVIDENCE[e] for e in evidence_ids))
...
class FixtureProvider:
    """ApplicationProvider backed by static in-memory fixtures.

    The fixtures never change, so the object graph is built once and shared.
    Nothing mutates the returned Application; per-run picks live in a SelectionStore.
    """

    def __init__(self) -> None:
        self._application = _build_application()
